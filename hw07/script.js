...
const game = {
    ...
    stop() {
        ...
        /* когда игра заканчивается, выводим количество очков
         * и запрещаем двигать змейку */
        document.getElementById('score-value').innerHTML = `Игра окончена, ваш счет: ${game.score}`;
...
const snake = {
    ...
    remove() {
      ...
      for (let coordinate of snake.parts) {
        document.querySelector(`.cell[data-top="${coordinate.top}"][data-left="${coordinate.left}"]`).classList.remove('snake');
      }
    }
};
